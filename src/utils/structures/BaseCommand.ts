...
import { nonPermEmbed } from "../components/embeds/nonPerms";
...
export default abstract class BaseCommand {
  ...
  async autoComplete(client: DiscordClient, interactions: AutocompleteInteraction, options: AutoCompleteOptions): Promise<void> {}
  ...
  async deferReply(
    ...
    if (msg instanceof Message) {
      return await msg[edit ? "edit" : "reply"](options as MessagePayload & MessageEditOptions);
    }
  ...
  async perms(client: DiscordClient, message: Message | CommandInteraction) {
    ...
    for await (const permission of this.permissions) {
      if (!member?.permissions.has(permission, true)) {
        if (message instanceof CommandInteraction && !message.isRepliable()) return;
        await message.reply({
          embeds: [nonPermEmbed],
        });
        return false;
  ...
  async execute(client: DiscordClient, message: Message | CommandInteraction, options: CommandOptions | InteractionOptions) {
  ...
  abstract run(client: DiscordClient, message: Message | CommandInteraction, options: CommandOptions | InteractionOptions): Promise<void>;
}
